...
function renderLiveCards(){
  ...
  ordered.forEach(member => {
    ...
    card.className = 'liveCard' + (st.live ? ' isLive' : '');
    card.innerHTML = `
      <div class="liveCardTop">
        ...
        <div>
          <h3 class="liveName">${member.display}</h3>
          <span class="liveState ${st.live ? 'on' : 'off'}">${st.live ? '🔴 EN DIRECT' : '⚫ HORS LIVE'}</span>
        </div>
      </div>
      <p class="liveDetails">${clean(st.text || (st.live ? 'Live en cours' : 'Pas en live pour le moment'))}</p>
      <div class="liveActions">
        <button onclick="showChannel('${member.name}', '${member.display}')">Regarder sur le site</button>
        <a href="https://www.twitch.tv/${member.name}" target="_blank">Ouvrir Twitch</a>
      </div>`;
    grid.appendChild(card);
  });
  const count = Object.values(liveStatuses).filter(x => x.live).length;
  const summary = $('liveSummary');
  if (summary) summary.textContent = count > 0 ? `🔴 ${count} membre(s) en live maintenant` : '⚫ Aucun membre en live pour le moment';
}

async function checkLiveStatus(member){
  // Service public sans clé API. Si Twitch/service bloque, le site reste fonctionnel et affiche hors live.
  const url = `https://decapi.me/twitch/uptime/${encodeURIComponent(member.name)}`;
  try {
    ...
    if (!offline && text) {
      liveStatuses[member.name] = {live:true, text:`En live depuis ${text}`};
    } else {
      liveStatuses[member.name] = {live:false, text:'Pas en live pour le moment'};
    }
  } catch(e) {
    liveStatuses[member.name] = {live:false, text:'Statut live indisponible, ouvre Twitch pour vérifier'};
  }
}

async function checkAllLiveStatus(manual){
  const summary = $('liveSummary');
  if (summary) summary.textContent = manual ? 'Actualisation des lives...' : 'Vérification des lives...';
  renderLiveCards();
